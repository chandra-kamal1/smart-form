...
export default function TableField({columns, rows}) {
  ...
  const attachment = (atms) =>{
    ...
    atms.forEach(element => {
      ats.push(<Attachment src={element} style={{float: 'left', marginRight: '5px'}}/>)
    });
    return ats;
  }
  return (
        ...
        <TableBody>
          ...
          {rows && rows.map((row) => (
            <TableRow key={row.name}>
                {columns && columns.ids.map((clm, index) => (
                    <TableCell component="th" scope="row">
                      {console.log(`clm Attachment`, clm)}
                      {console.log(`Row`, row)}
                        {clm == 'attList' && row[clm]!=null && row[clm]!="" ? 
                        // <Tooltip title={decodeURI(row[clm].toString().split("/")[row[clm].toString().split("/").length-1])}>
                        ...
                        attachment(row[clm])
                        : (
                          (clm == 'attachmentId' || clm.indexOf('Attachment') >= 0) && row[clm]!=null && row[clm]!=""  ? 
                          attachment([row[clm]])
                          :
                          
                          (row[clm]!=null && row[clm]!="" ? row[clm] : "-"))}
                    </TableCell>
                ))}
            </TableRow>
          ))}
